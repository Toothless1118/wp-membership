...
class es_widget_register extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		echo '<div class="es_top">';
		if ( ! empty( $es_title ) ) {
			/* dorin's change*/
			$html = '<figure class="entry-thumbnail">
                        <img src="/wp-content/themes/digital-freelancer/dist/images/connor.png" alt="Connor Black">
                    </figure>';
			echo $html.$args['before_title'] . $es_title . $args['after_title'];
		}
		...
		<div class="es_bottom">
			<form class="es_widget_form" es_form_id="es_widget_form">
				...
				<div  class="es_widget_form_wrapper">
				<?php if( $es_name == "YES" ) { ?>
				...
				<?php } ?>
					...
					<div class="es_textbox">
						<input type="text" id="es_txt_email" class="es_textbox_class" name="es_txt_email" onkeypress="if(event.keyCode==13) es_submit_page(event,'<?php echo $url; ?>')" value="" maxlength="225" placeholder="Enter your email address...">
					</div>
					<div class="es_button">
						<input type="button" id="es_txt_button" class="es_textbox_button es_submit_button" name="es_txt_button" onClick="return es_submit_page(event,'<?php echo $url; ?>')" value="<?php echo __( 'Sign Up', ES_TDOMAIN ); ?>">
					</div>
				</div>
