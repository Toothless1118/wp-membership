...
class es_cls_widget {
	public static function load_subscription($arr) {
		...
		$es .= '<div class="es_textbox input-group">';
			$es .= '<input type="text" id="es_txt_email_pg" class="form-control es_textbox_class" name="es_txt_email_pg" onkeypress="if(event.keyCode==13) es_submit_pages(event, '.$url.')" placeholder="Enter your email address..." value="" maxlength="225">';
		//$es .= '</div>';
		//$es .= '<div class="es_button">';
			$es .= '<input type="button" id="es_txt_button_pg" class="input-group-addon es_textbox_button es_submit_button" name="es_txt_button_pg" onClick="return es_submit_pages(event, '.$url.')" value="'.__( 'Subscribe', ES_TDOMAIN ).'">';
		$es .= '</div>';
