...
class MpdtMemberUtils extends MpdtBaseUtils {
  ...
  protected function extend_obj(Array $mbr) {
    ...
    $utils = MpdtUtilsFactory::fetch('transaction');
    $mbr['recent_transactions'] = array();
    foreach($rt as $i => $t) {
      $mbr['recent_transactions'][$i] = $utils->map_vars((array)$t->rec);
      $mbr['recent_transactions'][$i] = $utils->trim_obj($mbr['recent_transactions'][$i]);
    }
