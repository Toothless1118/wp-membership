...
class Widget extends \WP_Widget {
	...
	*/
    ...
    function itemHTML($instance,$current_post_id) {
        ...
        //$ret = '<li ';
        $ret .= '<div class="col-lg-4 mb-2">';
      	$ret .= '<a href="'.get_permalink().'"><div class="card card-inverse">';
        /*            
		// Current post
		...
		*/
		$ret .= '<div class="card-img-overlay">';
		// Title
		if(strlen(get_the_title()) > 40 ){
			$post_title = substr(get_the_title(), 0, 40).'...';
		} else {
			$post_title = get_the_title();
		}
		$ret .= '<h4 class="card-title">'. get_the_title() .'</h4>';	
		// Excerpt
        ...
        if (!isset($instance['excerpt_filters']) || $instance['excerpt_filters']) { // pre 4.7 widgets has filters on
			...
			if ($post->post_excerpt == '') {
				$text = get_the_content('');
				$text = '<p class="card-text">'.strip_shortcodes( $text ).'</p>';
		...
		$ret .= '<span>Read fully story <i class="fa fa-angle-right" aria-hidden="true"></i></span>';
		$ret .= '</div></a>';
		/*
		// Date
	...
	function widget($args, $instance) {
		...
		if ( !isset ( $instance["hide_if_empty"] ) || !$instance["hide_if_empty"] || $cat_posts->have_posts() ) {				
			...
			if (!(isset($instance['is_shortcode']) && $instance['is_shortcode'])) // the intenal id is needed only for widgets
				...
				echo '<ul>';
			*/
			echo '<div class="card-deck pt-3 mt-3">';
			if (!isset($instance['excerpt_filters']) || $instance['excerpt_filters']) // pre 4.7 widgets has filters on
			...
			<div>
				...
				<p>
					<label for="<?php echo $this->get_field_id("excerpt"); ?>" onchange="javascript:cwp_namespace.toggleExcerptPanel(this)">
						<input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("excerpt"); ?>" name="<?php echo $this->get_field_name("excerpt"); ?>"<?php //checked( (bool) $instance["excerpt"], true ); ?> checked/>
