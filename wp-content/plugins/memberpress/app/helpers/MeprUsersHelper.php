...
class MeprUsersHelper {
  ...
  public static function render_custom_field($line,$value='',$classes=array()) {
    ...
    switch($line->field_type) {
      ...
      case 'email':
        ...
        <!--dorin's change-->
          <?php
          switch($line->field_key) {
            case 'mepr_billing_address':
          ?>
            <label class="billing-label-text" for="basic-url">Billing Address</label>
            <div class="input-group setting-btn billing-label">
              <input type="<?php echo $line->field_type; ?>" name="<?php echo $line->field_key; ?>" id="<?php echo $line->field_key; ?>" class="form-control mepr-form-input <?php echo $class; ?>" value="<?php echo esc_attr($value); ?>" <?php echo $required_attr; ?> placeholder="Address1" />
            </div>
          <?php
              break;
            case 'mepr_address_2':
          ?>
            <div class="input-group setting-btn billing-label">
              <input type="<?php echo $line->field_type; ?>" name="<?php echo $line->field_key; ?>" id="<?php echo $line->field_key; ?>" class="form-control mepr-form-input <?php echo $class; ?>" value="<?php echo esc_attr($value); ?>" <?php echo $required_attr; ?> placeholder="Address2" />
            </div>
          <?php
              break;
            case 'mepr_city':
          ?>
            <div class="row">
              <div class="col-4">
                <div class="input-group setting-btn billing-label">
                  <input type="<?php echo $line->field_type; ?>" name="<?php echo $line->field_key; ?>" id="<?php echo $line->field_key; ?>" class="form-control mepr-form-input <?php echo $class; ?>" value="<?php echo esc_attr($value); ?>" <?php echo $required_attr; ?> placeholder="City" />
                </div>
              </div>
          <?php
              break;
          ...
          ?>
              <div class="col-4">
                <div class="input-group setting-btn billing-label">
                  <input type="<?php echo $line->field_type; ?>" name="<?php echo $line->field_key; ?>" id="<?php echo $line->field_key; ?>" class="form-control mepr-form-input <?php echo $class; ?>" value="<?php echo esc_attr($value); ?>" <?php echo $required_attr; ?>  placeholder="State" />
                </div>
              </div>
          
          <?php
              break;
            case 'mepr_payment_method':
          ?>
            <label class="billing-label-payment" for="basic-url">Payment Method</label>
            <div class="input-group setting-btn billing-label">
              <input type="<?php echo $line->field_type; ?>" name="<?php echo $line->field_key; ?>" id="<?php echo $line->field_key; ?>" class="form-control mepr-form-input <?php echo $class; ?>" value="<?php echo esc_attr($value); ?>" <?php echo $required_attr; ?> />
              <input type="submit" name="mepr-account-form" value="<?php _ex('Update', 'ui', 'memberpress'); ?>" class="btn dfbtn-green-setting mepr-submit mepr-share-button" />
            </div>
          <?php
              break;
            default:
          ?>
            <div class="input-group setting-btn billing-label mb-4">
              <input type="<?php echo $line->field_type; ?>" name="<?php echo $line->field_key; ?>" id="<?php echo $line->field_key; ?>" class="form-control mepr-form-input <?php echo $class; ?>" value="<?php echo esc_attr($value); ?>" <?php echo $required_attr; ?> />
            </div>
          <?php 
              break;
      ...
      case 'multiselect':
        ...
        $placeholder = "";
        if ($select_name == 'mepr_occupation') {
          $placeholder = '<option value="" disabled selected style="display: none"> What do you do?</option>';
        } elseif ($select_name == 'mepr_country'){
          $placeholder = '<option value="" disabled selected style="display: none"> Country</option>';
        } 
        ?>
        <?php
        if ($select_name == 'mepr_country') {
          ?>
              <div class="col-4">
                  <select name="<?php echo $select_name; ?>" id="<?php echo $line->field_key; ?>" class="select grayColor mepr-form-input mepr-select-field <?php echo $ms_class; ?> <?php echo $class; ?>" <?php echo $multiselect; ?> <?php echo $required_attr; ?>>
                  <?php
                    echo $placeholder;
        ...
        <?php } else {?>
        <p><select name="<?php echo $select_name; ?>" id="<?php echo $line->field_key; ?>" class="select grayColor mepr-form-input mepr-select-field <?php echo $ms_class; ?> <?php echo $class; ?>" <?php echo $multiselect; ?> <?php echo $required_attr; ?>>
          <?php
            echo $placeholder;
            foreach( $line->options as $o ) {
              if( $is_multi ) {
  ...
  public static function render_custom_fields( $product=null, $is_signup=false ) {
    ...
    foreach($custom_fields as $line) {
      /* dorin's change */
      if($product->signup_form_style == "style2"){
        if($line->field_key != "mepr_occupation"){
          continue;
        }
      }else{
  ...
  public static function render_custom_fields_billing( $product=null, $is_signup=false ) {
    MeprView::render('checkout/billing', get_defined_vars());
  }
