...
class MeprCheckoutCtrl extends MeprBaseCtrl {
  ...
  public function process_free_signup($df_redirct_url, $params) {
    $mepr_options = MeprOptions::fetch();
    
    // Validate the form post
    // $errors = MeprHooks::apply_filters('mepr-validate-signup', MeprUser::validate_signup($params, array()));
    // if(!empty($errors)) {
    ...
    if($is_existing_user || $this->df_signup_flag == 0) {
      $usr = MeprUtils::get_currentuserinfo();
    }
    else { // If new user we've got to create them and sign them in

      $usr = new MeprUser();
      $usr->user_login = ($mepr_options->username_is_email)?$params['user_email']:$params['user_login'];
      $usr->user_email = $params['user_email'];
      //Have to use rec here because we unset user_pass on __construct
      $usr->set_password($params['mepr_user_password']);
      
      
      try {

        $usr->store();
        //dorin's add - add meta
        if (isset($params['mepr_occupation']))
          update_user_meta($usr->ID, 'mepr_occupation', $params['mepr_occupation']);
        // Log the new user in
        wp_signon(
          array(
            'user_login'    => $usr->user_login,
            'user_password' => $params['mepr_user_password']
          ),
  ...
  public function process_signup_form() {
    ...
    // dorin's add
    $is_existing_user = MeprUtils::is_user_logged_in();
    if ($_REQUEST['df_redirct_url'] != "" &&  $this->df_signup_flag == 0 && !$is_existing_user) {
      $params = $_REQUEST;
      $this->df_signup_flag = 1;
      $usr = self::process_free_signup($_REQUEST['df_redirct_url'], $params);
    } else {
      // Check if the user is logged in already
      //$is_existing_user = MeprUtils::is_user_logged_in();
  ...
  public function process_payment_form() {
    if(isset($_POST['mepr_process_payment_form']) && isset($_POST['mepr_transaction_id']) && is_numeric($_POST['mepr_transaction_id'])) {
      ...
      //dorin's change for user meta fields saving
      MeprUsersCtrl::save_extra_profile_fields($txn->user_id);
      ///////////////
      if($txn->rec != false) {
        $mepr_options = MeprOptions::fetch();
        if(($pm = $mepr_options->payment_method($txn->gateway)) && $pm instanceof MeprBaseRealGateway) {
          ...
          if(empty($errors)) {
            ...
            MeprUtils::wp_redirect($mepr_options->thankyou_page_url("membership={$sanitized_title}&trans_num={$txn->trans_num}", $product->thank_you_url));
          }
          else {
            // Artificially set the payment method params so we can use them downstream
            // when display_payment_form is called in the 'the_content' action.
