...
class MpDripTags {
  public function __construct() {
    ...
    //Updating
    // add_action('mepr-txn-store',                    array($this, 'update_tags_txn_wrapper')); //Used mostly for testing quickly
  ...
  public function update_tags_txn_wrapper($txn) {
  ...
  public function update_tags($event_or_obj, $is_obj = false) {
    ...
    $contact  = $obj->user(true);

    //Member is active so let's not remove their tags
    if($contact->is_active_on_membership($obj)) {
      $this->maybe_add_subscriber_tags($obj, $contact);
    }
    else {
      $this->maybe_remove_subscriber_tags($obj, $contact);
    }
  }

  public function maybe_add_subscriber_tags($obj, $contact) {
    $enabled        = (bool)get_post_meta($obj->product_id, '_meprdriptags_membership_tags_enabled', true);
    $active_tags    = get_post_meta($obj->product_id, '_meprdriptags_membership_tags', true);
    $inactive_tags  = get_post_meta($obj->product_id, '_meprdriptags_membership_inactive_tags', true);
  ...
  public function maybe_remove_subscriber_tags($obj, $contact) {
    $enabled        = (bool)get_post_meta($obj->product_id, '_meprdriptags_membership_tags_enabled', true);
    $active_tags    = get_post_meta($obj->product_id, '_meprdriptags_membership_tags', true);
    $inactive_tags  = get_post_meta($obj->product_id, '_meprdriptags_membership_inactive_tags', true);
